...
//
// THERE WERE THREE OUTPUTS. The third was Cognito's managed-login branding at
// `infra/modules/auth/managed-login-settings.json`, which lives in
// insolvia-ai/insolvia and stayed there when these packages were extracted.
// That repo now reconciles it against the PUBLISHED `@insolvia-ai/tokens` —
// `tool/reconcile-cognito-branding.ts` over the `colors` export — rather than
// reaching into a tokens.json it no longer has. The two halves are joined by a
// published version, so a colour change ships from here first and is picked up
// there second. Do not re-add an output that writes outside this repo: nothing
// in a published package can be allowed to depend on a consumer's directory
// layout.
//
...
// workspace — there is no `npm run tokens` in the marketing site. It also
// keeps the banner toolchain-agnostic, so the generator and the way it is
// invoked can change without rewriting a generated byte.
const CSS_BANNER =
...
//
// It exists for ONE consumer — insolvia-ai/insolvia's Cognito branding
// reconciler — and for one blunt reason: `tokens.ts` is unreadable to it.
// Node's native type-stripping refuses any `.ts` file under `node_modules`
// (ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING), so a plain `node` script in a
// CONSUMER repo cannot import this package's TypeScript at all — the app's
// Metro and marketing's Vite can, which is why this only bites the one
// consumer that is neither.
//
// The alternative was for that repo to keep its own copy of the palette
// resolution — parseHex, mix, resolveAlias, valuesFor — and re-derive the same
// colours. That is the version worth refusing: the sign-in page's colours
// would then be computed by a second implementation of this file's blend
// maths, and would drift silently the first time one changed. Rendering the
// answer once, here, and shipping it as data keeps a single source.
//
...
function main(args: string[]): void {
  ...
  // Both outputs are rendered from scratch — neither reads its own previous
  // contents. The one output that was RECONCILED rather than rendered was the
  // Cognito document, and it left with the extraction (see the header note).

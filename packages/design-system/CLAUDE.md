...
- **The same rule now covers RADII and the type FAMILY, for a different
  ...
  `useNativeHeadingFamily()` / `useNativeBodyFamily()` / `useNativeMonoFamily()`
...
- **Every `Text` and `TextInput` a native leaf renders applies the body family
  unless it is a heading, mono, or a glyph in a fixed box.** `{ fontFamily:
  useNativeBodyFamily() }` in the render-time style array, placed before the
  caller's `style` so it can still be overridden. The hook returns `undefined`
  with no `ThemeProvider` — React Native ignores that — so the default is the
  platform sans it always was; what changed in 0.22.0 is that a consumer's
  `fonts.body` now reaches every control instead of none of them. A glyph
  drawn in a fixed box (a dismiss ×, a chevron, a tick, a pager arrow) is the
  carve-out and says so in a one-line comment; a `Text` nested inside another
  `Text` inherits and needs nothing.

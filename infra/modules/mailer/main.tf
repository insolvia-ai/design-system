...
#
# `insolvia_api` is this platform's one tenant, and the SigV4 caller role is
# looked up by name rather than hard-coded. The architecture: an ingress Lambda
# admits a message and hands it to SQS, a sender Lambda drains the queue and
# calls SES, and a feedback Lambda drains SES bounce/complaint/delivery
# notifications back into the message store. api_service reuses the same
# HTTP-API-behind-a-custom-domain shape this module establishes.
#
...
locals {
  ...
  # enough; this platform has only ever had one tenant.
...
# had one tenant/category family.
...
resource "aws_iam_role_policy" "sender" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      # From address — tighter than a blanket Resource = "*". SendRawEmail is
      # what SESv2's send_email(Content={"Raw": ...}) actually maps to (the
      # sender always sends raw, per services/mailer sender_lambda.py);
      # SendEmail is granted alongside it in case a non-raw send path is ever
      # added.
      {
        Effect   = "Allow"
...
# Same shape as api_service's HTTP API — REGIONAL custom domain, no CloudFront,
# for the same reasons documented there. The one difference: every route here
# requires AWS_IAM auth, since the only legitimate callers are other Insolvia
# service Lambdas signing with SigV4, never a browser.
...
# ─── Alerting (issue 6.7) ────────────────────────────────────────────────────
# Every alarm below is wired to aws_sns_topic.alarms, mirroring the house
# pattern in infra/modules/api_service/main.tf (aws_sns_topic.alarms +
...
# = 5 on all three queues) — a consistent failure, not a transient one. The
# keys name the actual DLQ resources (aws_sqs_queue.api_send_dlq /
# api_status_dlq / feedback_dlq).
...
# allowed_role_arns, etc). The dimension ServiceId "insolvia_api" is the
# service_registry_json key — see feedback_lambda.py's _metric() call, which
# titlecases the "reject" status into the "Reject" metric name this alarm
# watches.
...
# These two are also what AWS reviews before granting SES production access
# (issue 6.8) — staying under threshold here is a precondition for that
# request, not just an ops nicety. See services/mailer/docs/operations.md for
# the recovery steps when either fires.

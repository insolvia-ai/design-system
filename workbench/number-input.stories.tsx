 ...
 *
 * Not to be confused with the row's several rings by design — the field and
 * each stepper own one apiece, since each is separately focusable. The
 * invariant here is about a single FOCUSED element: it paints one ring, and
 * that ring is ours.
 *
...
export const NoBrowserFocusRing: Story = {
  ...
  play: async ({ canvasElement, step }) => {
    ...
    await step('native leaf: the row rings, the input inside it does not', async () => {
      const input = native.getByRole('spinbutton', { name: 'Quantity' });

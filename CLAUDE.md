...
design system. It follows a conventional infra layout (OIDC to AWS, Route53 +
ACM + CloudFront, `<project>-<thing>-<env>` naming), on a Dart/Flutter stack.
...
reverse), plus backend services in `services/`.
...
| `services/api/` | The backend API — Flask + Mangum on Lambda (decision D6 in `docs/MVP_PLAN.md`). Layered `core/api/adapters/entrypoints`, with the dependency direction machine-enforced by `tests/test_architecture.py`. Brokers **all** AWS access for every client — see `docs/adr/0001-client-stays-dumb-trust-boundary.md`. **Not a pub workspace member** (Python, not Dart). | Python (`insolvia_api`) |
| `services/mailer/` | The transactional email service (milestone 7). Same layered `core/api/adapters/entrypoints` split as `services/api`, with its own `tests/test_architecture.py` enforcing the dependency direction. Owns admission, S3 manifests, SQS, and SES delivery behind the `MAILER_*`-prefixed env vars; local dev delivers only to a Mailpit container (`services/mailer/docker-compose.yml`), never a real relay. **Not a pub workspace member** (Python, not Dart). | Python (`insolvia_mailer`) |
...
- **AWS:** Insolvia runs in its **own dedicated AWS account** (`521762924626`). Resources are namespaced by the `insolvia` project and by environment. (This is why `infra/envs/shared` **creates** the GitHub OIDC provider rather than reading it as a `data` source — there is exactly one per account.)
...
Insolvia runs **`staging` and `prod`** for
...
(us-east-1, DNS-validated), and the SES domain identity + mail DNS. It is
applied by CI like the other environments.

The **trust anchor** — the account-level **GitHub OIDC provider** and the
`insolvia-github-actions` deploy role with its permissions policy — lives in a
**separate root, `infra/envs/ci-trust/`**, that CI never applies. The reason
is `DenySelfPrivilegeEscalation`: the deploy role's policy forbids the role
from editing its own permissions, so a CI apply of that policy (run *as* the
role) fails by design. Making "human-applied" a property of the directory —
there is deliberately no `ci-trust-*.yml` workflow — is cleaner than burying
the gate inside `shared`, and it lets `shared` be freely CI-applied. Changes to
the deploy role's permissions are applied by a human admin with
`scripts/apply-ci-trust.sh`. (Insolvia has its own dedicated AWS account —
`521762924626` — so `ci-trust` creates the OIDC provider itself; there is
...
deliberately *not* a broad port of Base UI wrappers.
...
if CODEOWNER review is wanted (`.github/CODEOWNERS` already assigns a code
owner, but the ruleset does not currently enforce it — see *Branch Conventions*).
...
- **Not** enforced today, despite `.github/CODEOWNERS` existing:
  ...
  requests the code owner's review, it does not gate the merge.
